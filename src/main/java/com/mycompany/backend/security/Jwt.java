  ...
  public static String createAccessToken(String mid, String authority) {
    ...
    try {
      accessToken = Jwts.builder()
          ...
          // 페이로드 설정
          .setExpiration(new Date(new Date().getTime() + ACCESS_TOKEN_DURATION)) // setExpiration은 Date 타입을 값으로 받기 때문에 더한 뒤에 다시 감싼다.
          .claim("mid", mid)
          .claim("authority", authority) // ROLE_USER
          // 서명 설정
          .signWith(SignatureAlgorithm.HS256, JWT_SECRET_KEY.getBytes("UTF-8")).compact();
    } catch (Exception e) {
...
//RefreshToken 생성
  public static String createRefreshToken(String mid, String authority) {
    ...
    try {
      refreshToken = Jwts.builder()
          ...
          .setHeaderParam("alg", "HS256").setHeaderParam("typ", "JWT")
          // 페이로드 설정
          .setExpiration(new Date(new Date().getTime() + REFRESH_TOKEN_DURATION)) // setExpiration은 Date 타입을 값으로 받기 때문에
                                                                                  // 더한 뒤에 다시 감싼다.
          .claim("mid", mid).claim("authority", authority) // ROLE_USER
          // 서명 설정
          .signWith(SignatureAlgorithm.HS256, JWT_SECRET_KEY.getBytes("UTF-8")).compact();
    } catch (Exception e) {
      log.error(e.getMessage());
  ...
  public static boolean validateToken(String token) {
    ...
    try {
      result = Jwts.parser()
          ...
          .parseClaimsJws(token) // Jws(클레임) 객체 얻기
          .getBody() // 클레임 객체 얻기
          .getExpiration() // 만료 기간 얻기
          .after(new Date()); // 받아온 값이 현재 시간을 넘었는지 체크
    } catch (Exception e) {
      log.error(e.getMessage());
    }
  ...
  //Bearer xxxxxxxxxxxxxx.xxxxxxxxxxxx.xxxxxxxxxxx
  public static String getAccessToken(String authoriztion) {
    String accessToken = null;
    if(authoriztion != null && authoriztion.startsWith("Bearer ")) {
      accessToken = authoriztion.substring(7);
    }
    return accessToken;
  }
